...
//1)js객체 => 일반객체 > 배열객체
//2)일반객체
//장점 : key & value 값을 가지고 있는 프로퍼티 => 데이터를 체계적으로 보관, 관리, 사용
//단점 : for...in => 객체 안에 있는 value값만을 찾아와서 직접적으로 반복문을 돌릴 수 있는 방법x
//단점 : 객체안에 구성되어 있는 아이템들은 배열처럼 인덱스 번호를 할당받지 못함
//3)배열객체
//장점 : 배열안에 포함되어 있는 아이템들이 고유의 인덱스값을 가지고 있으며, for문을 활용한 반복문으로 업무에 효율성 극대화
//단점 : 객체처럼 key & value 형태로 구성된 체계적인 데이터 관리x

//해결책 : 일반객체 & 배열객체의 장점만 모아서 하나로 만들어 주면 어떨까 = > Map
...
// console.log(myCup);
// console.log(myCup.size); //아이템의 갯수
// console.log(myCup.get("color"));
// console.log(myCup.has("color")); //해당 key 존재 여부
...
//일반객체 +배열객체+Map 객체+데이터 중복 막을 수 있는 = Set() 객체
