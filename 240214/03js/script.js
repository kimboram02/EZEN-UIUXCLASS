...
btn.addEventListener("click", function (e) {
  e.preventDefault();
  const originPrice = document.querySelector("#price").value; //value => input 값을 찾아와라
  const rate = document.querySelector("#rate").value;
  const savedPrice = originPrice * (rate / 100);
  const resultPrice = originPrice - savedPrice;
  showResult.innerHTML = `상품의 원래 가격은 ${originPrice}이고, 할인율은 ${rate}% 입니다. ${savedPrice}원을 절약한 ${resultPrice}원에 구매할 수 있습니다`;
});
